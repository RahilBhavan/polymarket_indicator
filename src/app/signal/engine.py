...
def _recommended_and_liquidity_warning(
    ...
    if direction == "YES":
        rec = recommended_size_usd(model_p, market_p, bankroll_usd, quote.max_safe_size_usd)
    elif direction == "NO":
        price_no = max(0.01, min(0.99, 1.0 - market_p))
        rec = recommended_size_usd(1.0 - model_p, price_no, bankroll_usd, quote.max_safe_size_usd)
    else:
        return 0.0, None
