...
def test_flatten_event_markets() -> None:
    ...
    events = [
        {
            "id": "e1",
            "markets": [{"id": "m1", "conditionId": "c1"}, {"id": "m2", "conditionId": "c2"}],
        },
...
def test_pick_latest_live_market_accepts_snake_case_dates() -> None:
    ...
    markets = [
        {
            "id": "m1",
            "end_date": "2026-01-31T20:00:00Z",
            "event_start_time": "2026-01-31T19:00:00Z",
        },
    ]
    picked = pick_latest_live_market(markets, now=now)
    assert picked is not None
...
def test_parse_updown_market_returns_none_when_missing_outcomes() -> None:
    ...
    assert (
        parse_updown_market(
            {"conditionId": "0x", "outcomes": "[]", "clobTokenIds": []},
            up_label="Up",
            down_label="Down",
        )
        is None
...
async def test_select_btc_15m_updown_market_no_events() -> None:
    ...
    with patch(
        "app.polymarket.selection_15m.fetch_events_by_series_id", new_callable=AsyncMock
    ) as mock_fetch:
